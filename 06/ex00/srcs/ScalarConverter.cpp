...
{
    return (d > std::numeric_limits<char>::min() && d < std::numeric_limits<char>::max() && end[0] == '\0' && std::fmod(d, 1.0) == 0);
}

bool ScalarConverter::isCastableToInt(double d, char* end)
{
    return (d > std::numeric_limits<int>::min() && d < std::numeric_limits<int>::max() && end[0] == '\0' && std::fmod(d, 1.0) == 0);
}

bool ScalarConverter::isCastableToFloat(double d, char* end)
{
    return (d > -std::numeric_limits<float>::max() && d < std::numeric_limits<float>::max() && end[0] == 'f');
}
...
{
	std::cout << "char:\t";
	if (n < std::numeric_limits<char>::min() || n > std::numeric_limits<char>::max() || isnan(n))
	{
...
{
	std::cout << "int:\t";
	if (n < std::numeric_limits<int>::min() || n > std::numeric_limits<int>::max() || isnan(n))
	{
		std::cout << "impossible" << std::endl;
		return;
	}
...
{
	std::cout << "float:\t";
	if ((n < -std::numeric_limits<float>::max() || n > std::numeric_limits<float>::max()) && !isinf(n))
	{
		std::cout << "impossible" << std::endl;
		return;
	}
...
{
	std::cout << "double:\t";
	if ((n < -std::numeric_limits<double>::max() || n > std::numeric_limits<double>::max()) && !isinf(n))
	{
		std::cout << "impossible" << std::endl;
		return;
	}
